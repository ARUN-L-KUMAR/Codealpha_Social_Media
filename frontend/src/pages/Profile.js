...
const Profile = () => {
  ...
  const fetchUserPosts = async () => {
    try {
      ...
      const userPosts = await postsAPI.getUserPosts(profileUserId);
      setPosts(userPosts);
    } catch (error) {
  ...
  const handleFollow = async () => {
    try {
      if (following) {
        await usersAPI.unfollowUser(profileUserId);
        setFollowing(false);
  ...
  const tabs = [
    ...
    { id: 'media', label: 'Media', count: posts.filter(post => post.images?.length > 0).length },
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50 to-purple-100 dark:from-gray-900 dark:via-gray-800 dark:to-purple-900">
      <div className="max-w-4xl mx-auto pt-20 px-4">
        ...
        <div className="pb-20">
          ...
          {activeTab === 'media' && (
            <div>
              {posts.filter(post => post.images?.length > 0).length > 0 ? (
                <div className="grid grid-cols-2 md:grid-cols-3 gap-2 md:gap-4">
                  {posts.filter(post => post.images?.length > 0).flatMap(post => post.images).map((image, index) => (
                    <div key={index} className="aspect-square overflow-hidden rounded-lg shadow">
                      <img src={image} alt="" className="w-full h-full object-cover hover:scale-105 transition-transform duration-300 cursor-pointer" />
                    </div>
                  ))}
                </div>
              ) : (
